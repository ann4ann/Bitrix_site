...
<?if($arResult["FORM_TYPE"] == "login"):?>
	...
	<!-- код для работы входа через соцсети -->
	<?if($arResult["AUTH_SERVICES"]):?>
	<?
	...
	<div class="row form-group">
		<div class="col-md-12 mb-3 mb-md-0">
			...
			<div>
				...
				<p><a href="<?=$arResult["PROFILE_URL"]?>" title="<?=GetMessage("AUTH_PROFILE")?>"><?=GetMessage("AUTH_PROFILE")?></a><p>
			</div>
				...
				<a href="<?=$APPLICATION->GetCurPageParam("logout=yes"), array(
					...
					"forgot_password",
					"change_password",
					)?>" class="btn btn-primary  py-2 px-4 rounded-0"><?=GetMessage("AUTH_LOGOUT_BUTTON")?></a>
		</div>
	</div>
</div>
